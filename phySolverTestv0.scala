...
//  }
  ...
  def timeToHit(speed: Int, initd: Vector, endd: Vector, cache: List[List[List[String]]]): Double = {
    var code_indx = 0
  ...
  println(timeToHit(speed, initd, endd, cache3))
  val cache4 = List(
    List(List("endd"), List("initd"), List()),
    List(List("speed"), List(), List()),
    List(List("dist"), List(), List())
  )
  println(timeToHit(speed, initd, endd, cache4))
}